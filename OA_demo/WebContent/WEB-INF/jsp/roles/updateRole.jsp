...
<body>
 <center> <h1>修改角色权限</h1></center>
	<div class="page-container">
	<form action="<%=basePath %>role/updateRole" method="post" class="form form-horizontal" id="Form" >
		...
		<input type="hidden" value="${role.rid }" name="rid">
		 ...
		 <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
					  ...
					  <!-- 遍历所有角色的权限-->
					  <c:forEach items="${privilleges}" var="role">					  
					 <input type="checkbox" name="pid" value="${role.pid }" 
					  <c:forEach items="${privillegeList}" var="ro">
						<c:if test="${role.pid==ro.pid }">checked="checked"</c:if>
					  </c:forEach>					 
					> ${role.pname }<br>
				      </c:forEach>
			</div>
